...
import constat # NOTE: added this
import pandas as pd
...
def load_model(name1, name2):
    if "davinci" in name1:
        ...
        model1, tokenizer1 = constat.load_model(name1, return_tokenizer=True, trust_remote_code=False) # NOTE: changed this
        model1.eval()
    ...
    if "davinci" in name2:
        ...
        model2, tokenizer2 = constat.load_model(name2, return_tokenizer=True, trust_remote_code=False) # NOTE: changed this
        model2.eval()
...
def generate_in_batches(model, tokenizer, input_ids, batch_size, max_new_tokens, num_return_sequences, generate_args):
    # replicate the input_ids to match num_return_sequences
...
def sample_generation(sentence, model, tokenizer, args):
    ...
    output = generate_in_batches(model, tokenizer, input_ids, batch_size=50, max_new_tokens=len(sentence.split())-half_sentence_index, num_return_sequences=args['num_z'],
...
if __name__ == '__main__':
    ...
    # NOTE: Changed the following 20 lines completely
    ...
    if "jsonl" in args.data:
        ...
        dataset = pd.read_csv("../data/contamination/arc/contamination.csv", converters={'choices': literal_eval})
        data = process_arc(dataset)
    elif args.data == "gsm8k":
        dataset = pd.read_csv("../data/contamination/gsm8k/contamination.csv")
        data = process_gsm8k(dataset)
    elif args.data == 'hellaswag':
        dataset = pd.read_csv("../data/contamination/hellaswag/contamination.csv")
        data = process_hellaswag(dataset)
